...
/**
 * [Suposição] PUT /athlete/{id}/events/{event_id} — endpoint REST padrão
 * do Intervals.icu para edição in-place de um evento existente.
 * Usado para substituir treinos que o próprio coach empurrou anteriormente.
 */
export async function updateWorkoutInIntervals(params: {
  eventId: number;
  name: string;
  description: string;
  dateStr: string;
  ...
  const auth = Buffer.from(`API_KEY:${apiKey}`).toString("base64");
  const res = await fetch(`${INTERVALS_API_BASE}/athlete/${athleteId}/events/${params.eventId}`, {
    method: "PUT",
    headers: {
      Authorization: `Basic ${auth}`,
      "Content-Type": "application/json",
  ...
  if (!res.ok) {
    throw new Error(`Falha ao actualizar treino no Intervals.icu: ${res.status} ${await res.text()}`);
  }
  const data = (await res.json()) as { id: number };
  return { id: data.id, url: `https://intervals.icu/calendar` };
}
